cat <<EOF | curl -X POST -H "Content-Type: application/json" -u "admin:smartvm" --data @- "localhost:3000/api/providers/9"
{
"action": "edit",
"resource" : {
    "connection_configurations": [
         {
           "endpoint": {
             "role": "default",
             "hostname": "ocp-master01.10.35.48.189.xip.io",
             "port": 8443
           },
           "authentication": {
             "role": "bearer",
             "auth_key": "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..UhGy45pjWTak0NznD1v2rQtbf-XYis_5_ZKqVL0o7_4NFc69hsZZ4i_pSl2Pb4qsyNhWyZRY0JpKmBDz8CXIiZTb92laqrg_QQO4qMxo1nn2dHq_8nsMfdMAzR_dDzbUAlyPjhLMqUU9CecWzTodZ_PxLEfjZpMw8qIVAtca5fl5xgK1TbvnBdbnD_wK57PphadAZ1MCUdgzNgrs58WC59R1dv0lCL15UEXxamowLDZy1zWOG-WiHxFz5wN5iN7KCkPMnABOLoQ4k53kg-4sZzwqCPziUqyWm0mer2TwLzhqkuztzJnQ9AQBtt1kWvFKmPfGDUZ91bfGNfKeyd1gFw"
           }
         },
         {
           "endpoint": {
             "role": "prometheus-alerts",
             "hostname": "alerts-openshift-metrics.10.35.48.190.xip.io",
             "port": 443
           },
           "authentication": {
             "role": "prometheus-alerts",
             "auth_key": "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..UhGy45pjWTak0NznD1v2rQtbf-XYis_5_ZKqVL0o7_4NFc69hsZZ4i_pSl2Pb4qsyNhWyZRY0JpKmBDz8CXIiZTb92laqrg_QQO4qMxo1nn2dHq_8nsMfdMAzR_dDzbUAlyPjhLMqUU9CecWzTodZ_PxLEfjZpMw8qIVAtca5fl5xgK1TbvnBdbnD_wK57PphadAZ1MCUdgzNgrs58WC59R1dv0lCL15UEXxamowLDZy1zWOG-WiHxFz5wN5iN7KCkPMnABOLoQ4k53kg-4sZzwqCPziUqyWm0mer2TwLzhqkuztzJnQ9AQBtt1kWvFKmPfGDUZ91bfGNfKeyd1gFw"
           }
         }

       ]
    }
}
EOF


